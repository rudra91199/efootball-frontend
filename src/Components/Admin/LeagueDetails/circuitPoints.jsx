...
const CircuitPoints = ({ tournamentId }) => {
  const {
    ...
    queryFn: () => {
      return API.get(
        `/circuit-points/generateFinalSeedingLeaderboard/${tournamentId}`,
        {
          headers: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Circuit Points</h1>
          <p className="text-muted-foreground mt-1">
            Track your performance across tournament stages
          </p>
        </div>
      </div>
      ...
      <div className="bg-white/5 sm:p-6 rounded-lg p-1 border-2 border-border/70">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm mr-2 text-muted-foreground">
            Total Players: {finalPhaseLeaderboard?.length}
          </div>
        </div>
                ...
                >
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      <img
                        src={
                          player?.playerInfo?.logo?.url || "/placeholder.svg"
                        }
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-background/50 p-4 rounded-lg text-center">
            ...
            <p className="text-2xl font-bold text-blue-400">
              {
                finalPhaseLeaderboard?.find(
                  (player) => player.phase1_points == 4
                )?.playerInfo.name
              }
            </p>
          </div>
